...
def record_position(position_history: list, topic: str, stance: str, timestamp: str = ""):
    """Record a stance on a topic. Caps history at 30 entries."""
    entry = {"topic": topic, "stance": stance}
    if timestamp:
        entry["posted_at"] = timestamp
    position_history.append(entry)
    return position_history[-30:]


def get_relevant_positions(position_history: list, post_text: str, n: int = 3) -> list[dict]:
